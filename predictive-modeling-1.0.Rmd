...
The adage  _hind sight is 20/20_ is often used to refer how we would have prepared differently had we known certain occurrences. Most of us, at one point or another,  may have said 'had I known x, I would have done y'. For example, had I known it was going to rain, I would have worn my rain jacket.  Had I known event x happened with my customer, I would have given him this option or discount, or chosen this route.  This 'given this, then that' scenario simulation is possible if we consider, what is known in the statistics world, [conditional probability](https://en.wikipedia.org/wiki/Conditional_probability).  Applying 'conditional probability' increases accuracy to our response by  noticeable magnitude.   **“Conditional probability tells us how to update our belief in light of a new evidence that comes in."[2]**  Therefore, applying conditional statistics, based on actual data, will give us our best chance to understand and make decisions that are important.
...
us_state_region <- read.csv("~/Documents/DataScience/Blogs/Blog7/data/us_state_region.csv")
...
# dataset 3
# download from NOAA
us_weather <- tbl_df(read.csv("~/Documents/DataScience/Blogs/Blog7/data/us-cities-monthly-weather-noaa-2.csv"))
...
Before even starting the statistical analysis, from the table and the map (click around different cities), we can observe that the average temperature generally warmer in south and south west vs the Midwest or east, for instance.  
...
```
**_Table 2: The table can be explained as follows:- for 21 degree latitude, in the month of April, on average, the temperature is between 72.6 and 78.3 degree Fahrenheit._**
